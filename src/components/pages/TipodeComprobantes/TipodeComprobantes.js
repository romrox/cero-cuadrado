...
export default function TipoDeComprobante() {
  ...
  const [TipoDeComprobante, setTipoDeComprobante] = useState([]);
  ...
  const [newTipoDeComprobanteName, setNewTipoDeComprobanteName] = useState("");
  ...
  useEffect(() => {
    fetch("https://localhost:7244/api/Presupuesto/GetAllTipoDeComprobante")
      .then((response) => response.json())
      .then((data) => setTipoDeComprobante(data))
      .catch((error) =>
        console.error("Error fetching TipoDeComprobante", error)
      );
  }, []);
  ...
  const handleDelete = (id) => {
    ...
    fetch(
      `https://localhost:7244/api/Presupuesto/DeleteTipoDeComprobante?id=${id}`,
      {
        method: "DELETE",
      }
    )
      .then((response) => {
        if (response.ok) {
          ...
          setTipoDeComprobante(
            TipoDeComprobante.filter((item) => item.id !== id)
          );
        } else {
          throw new Error("Error al eliminar TipoDeComprobante");
        }
      })
      .catch((error) =>
        console.error("Error al eliminar TipoDeComprobante:", error)
      );
  };

  const handleEdit = (id, newName) => {
    fetch(
      `https://localhost:7244/api/Presupuesto/TipoDeComprobanteUpDate?TipoDeComprobanteId=${id}`,
      {
        method: "PUT",
        headers: {
      ...
      .then((response) => {
        if (response.ok) {
          ...
          fetch(
            "https://localhost:7244/api/Presupuesto/GetAllTipoDeComprobante"
          )
            .then((response) => response.json())
            .then((data) => setTipoDeComprobante(data))
            .catch((error) =>
          ...
          throw new Error("Error al actualizar TipoDeComprobante");
        }
      })
      .catch((error) =>
        console.error("Error al actualizar TipoDeComprobante:", error)
      );
  };

  const handleCreate = (newName) => {
    fetch(`https://localhost:7244/api/Presupuesto/TipoDeComprobante`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      ...
      .then((response) => {
        if (response.ok) {
          ...
          throw new Error("Error al crear TipoDeComprobante");
        }
      })
      .catch((error) =>
        console.error("Error al crear TipoDeComprobante:", error)
      );
  };
      ...
      >
        ...
        >
            ...
            <TableRow sx={{ background: " #424242", color: "#fff" }}>
              ...
              >
                ...
                >
                  <Box sx={style}>
                    <TextField
                      value={newTipoDeComprobanteName}
                      onChange={(e) =>
                        setNewTipoDeComprobanteName(e.target.value)
                      }
                      margin="normal"
                      fullWidth
                      variant="filled"
                      className="newTipoDeComprobanteName"
                      name="setNewTipoDeComprobanteName"
                      type="newTipoDeComprobanteName"
                      id="newTipoDeComprobanteName"
                    ...
                    <Button
                      onClick={() => {
                        handleCreate(newTipoDeComprobanteName);
                        handleClose2();
                      }}
                      type="submit"
                      fullWidth
          ...
          <TableBody sx={{ background: " #424242", color: "#fff" }}>
            {TipoDeComprobante.filter((TipoDeComprobante) =>
              TipoDeComprobante.name
                .toLowerCase()
                .includes(searchTerm.toLowerCase())
            )
              .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
              .map((TipoDeComprobante) => (
                <TableRow
                  hover
                  role="checkbox"
                  tabIndex={-1}
                  key={TipoDeComprobante.id}
                >
                  <TableCell
                    sx={{
                      background: " #424242",
                  ...
                  >
                    {formatFecha(TipoDeComprobante.data)}
                  </TableCell>
                  <TableCell
                    sx={{
                      background: " #424242",
                  ...
                  >
                    {formatFecha(TipoDeComprobante.modifiedDate)}
                  </TableCell>
                  <TableCell align="center">
                    <Box>
                      <Button
                        style={{ color: "#fff" }}
                        onClick={() => handleDelete(TipoDeComprobante.id)}
                      >
                        <DeleteIcon />
                      </Button>
                      <Button
                        style={{ color: "#fff" }}
                        onClick={() => {
                          setSelectedItemId(TipoDeComprobante.id);
                          handleOpen(TipoDeComprobante.name);
                        }}
                      >
                        <EditIcon />
                      </Button>

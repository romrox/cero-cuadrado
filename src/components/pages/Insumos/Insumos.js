...
const columns = [
  ...
  { id: "cost", label: "Costo:", minWidth: 170, align: "center" },
  { id: "Unidad", label: "Unidad:", minWidth: 170, align: "center" },
...
export default function Insumos() {
  ...
  const [newInsumosName, setNewInsumosName] = useState("");
  ...
  useEffect(() => {
    fetch("https://localhost:7244/api/Presupuesto/GetAllInsumos")
  ...
  useEffect(() => {
    fetch("https://localhost:7244/api/Presupuesto/GetAllUnidades")
      .then((response) => response.json())
  ...
  const handleDelete = (id) => {
    ...
    fetch(`https://localhost:7244/api/Presupuesto/DeleteInsumos?id=${id}`, {
      method: "DELETE",
    })
      .then((response) => {
        if (response.ok) {
  ...
  const handleEdit = (id, newName) => {
    fetch(
      `https://localhost:7244/api/Presupuesto/InsumosUpDate?InsumosId=${id}`,
      {
        method: "PUT",
        headers: {
  ...
  const handleCreate = (newName) => {
    fetch(`https://localhost:7244/api/Presupuesto/Insumos`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      ...
      >
        ...
        >
              ...
              >
                ...
                <Modal open={open} onClose={handleClose}>
                  <Box sx={style}>
                    ...
                    <TextField
                      value={NewUnidades}
                      onChange={(e) => setNewUnidades(e.target.value)}
                      margin="normal"
                      fullWidth
                      variant="filled"
                      className="NewUnidades"
                      name="NewUnidades"
                      label="Unidades"
                      type="NewUnidades"
                      id="NewUnidades"
                    ...
                    <Button
                      onClick={() => {
                        handleEdit(selectedItemId, newItemName, NewUnidades);
                        handleClose();
                      }}
                      type="submit"
                      fullWidth
                ...
                >
                  <Box sx={style}>
                    <TextField
                      value={newInsumosName}
                      onChange={(e) => setNewInsumosName(e.target.value)}
                      margin="normal"
                      fullWidth
                      variant="filled"
                      className="newInsumosName"
                      name="newInsumosName"
                      type="newInsumosName"
                      id="newInsumosName"
                    ...
                    <Button
                      onClick={() => {
                        handleCreate(newInsumosName, NewUnidades);
                        handleClose2();
                      }}
                      type="submit"
                      fullWidth
          ...
          <TableBody sx={{ background: " #424242", color: "#fff" }}>
            {Insumos.filter((Insumos) =>
              Insumos.name.toLowerCase().includes(searchTerm.toLowerCase())
            )
              .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
              .map((Insumos) => (
                <TableRow hover role="checkbox" tabIndex={-1} key={Insumos.id}>
                  <TableCell
                    sx={{
                      background: " #424242",
                      color: "#fff",
                  ...
                  >
                    {formatFecha(Insumos.modifiedDate)}
                  </TableCell>
                  <TableCell align="center">
                    <Box>
                      <Button
                        style={{ color: "#fff" }}
                        onClick={() => handleDelete(Insumos.id)}
                      >
                        <DeleteIcon />
                      </Button>
                      <Button
                        style={{ color: "#fff" }}
                        onClick={() => {
                          setSelectedItemId(Insumos.id);
                          handleOpen(Insumos.name);
                        }}
                      >
                        <EditIcon />
                      </Button>

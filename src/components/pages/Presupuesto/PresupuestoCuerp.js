...
export default function PresupuestoCuerp() {
  ...
  const [newCentroDeCostosName, setNewCentroDeCostosName] = useState("");
  ...
  useEffect(() => {
    fetch("https://localhost:7244/api/Presupuesto/GetAllCentroDeCostos")
      .then((response) => response.json())
      .then((data) => setCentrosDeCostos(data))
      .catch((error) =>
  ...
  const handleDelete = (id) => {
    ...
    fetch(
      `https://localhost:7244/api/Presupuesto/DeletCentroDeCostos?id=${id}`,
      {
        method: "DELETE",
      }
    )
      .then((response) => {
        if (response.ok) {
          ...
          setCentrosDeCostos(centrosDeCostos.filter((item) => item.id !== id));
        } else {
  ...
  const handleEdit = (id, newName) => {
    fetch(
      `https://localhost:7244/api/Presupuesto/CentroDeCostosUpDate?CentroDeCostosId=${id}`,
      {
        method: "PUT",
        headers: {
      ...
      .then((response) => {
        if (response.ok) {
          ...
          throw new Error("Error al actualizar el centro de costos");
        }
      })
      .catch((error) =>
  ...
  const handleCreate = (newName) => {
    fetch(`https://localhost:7244/api/Presupuesto/CentroDeCostos`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      ...
      .then((response) => {
        if (response.ok) {
          ...
          throw new Error("Error al crear el centro de costos");
        }
      })
      .catch((error) =>
            ...
            <TableRow sx={{ background: " #424242", color: "#fff" }}>
              ...
              >
                ...
                >
                  <Box sx={style}>
                    <TextField
                      value={newCentroDeCostosName}
                      onChange={(e) => setNewCentroDeCostosName(e.target.value)}
                      margin="normal"
                      fullWidth
                      variant="filled"
                      className="newCentroDeCostosName"
                      name="setNewCentrnewCentroDeCostosNameoDeCostosName"
                      type="newCentroDeCostosName"
                      id="newCentroDeCostosName"
                      label="Nombre del nuevo Centro de Costos"
                      InputLabelProps={{
                        sx: {
                          color: "#fff",
                          "&::after": {
                    ...
                    <Button
                      onClick={() => {
                        handleCreate(newCentroDeCostosName);
                        handleClose2();
                      }}
                      type="submit"
                      fullWidth
            ...
            {centrosDeCostos
              ...
              .map((centroDeCostos) => (
                ...
                >
                  ...
                  >
                    {formatFecha(centroDeCostos.data)}
                  </TableCell>
                  <TableCell
                    sx={{
                      background: " #424242",
                  ...
                  >
                    {formatFecha(centroDeCostos.modifiedDate)}
                  </TableCell>
                  <TableCell align="center">
                    <Box>
                      <Button
                        style={{ color: "#fff" }}
                        onClick={() => handleDelete(centroDeCostos.id)}
                      >
                        <DeleteIcon />
                      </Button>
                      <Button
                        style={{ color: "#fff" }}
                        onClick={() => {
                          setSelectedItemId(centroDeCostos.id);
                          handleOpen(centroDeCostos.name);
                        }}
                      >
                        <EditIcon />
                      </Button>

...
export default function Proveedores() {
  ...
  const [newProveedorCuit, setNewProveedorCuit] = useState("");
  ...
  const handleOpen = (name, cuit) => {
    setNewProveedorName(name);
    setNewProveedorCuit(cuit);
    setOpen(true);
  ...
  useEffect(() => {
    fetch("https://localhost:7244/api/Presupuesto/GetAllProveedores")
  ...
  const handleDelete = (id) => {
    ...
    fetch(`https://localhost:7244/api/Presupuesto/DeleteProveedores?id=${id}`, {
      method: "DELETE",
    })
      .then((response) => {
        if (response.ok) {
          // Actualiza el estado para reflejar el cambio
          setProveedor(Proveedor.filter((item) => item.id !== id));
        } else {
  ...
  const handleEdit = (id, newName, newCuit) => {
    fetch(
      `https://localhost:7244/api/Presupuesto/ProveedoresUpDate?ProveedoresId=${id}`,
      {
        method: "PUT",
        headers: {
  ...
  const handleCreate = (newName, newCuit) => {
    fetch(`https://localhost:7244/api/Presupuesto/Proveedores`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
              ...
              >
                ...
                <Modal open={open} onClose={handleClose}>
                  <Box sx={style}>
                    <TextField
                      ...
                      fullWidth
                      variant="filled"
                      className="newProveedorName"
                      name="setProveedorName"
                      type="newProveedorName"
                      id="newProveedorName"
                    ...
                    />
                    <TextField
                      value={newProveedorCuit}
                      onChange={(e) => setNewProveedorCuit(e.target.value)}
                      margin="normal"
                      fullWidth
                      variant="filled"
                      name="newProveedorCuit"
                      label="CUIT"
                      type="number"
                      id="newProveedorCuit"
                      InputLabelProps={{
                ...
                >
                  <Box sx={style}>
                    ...
                    <Button
                      onClick={() => {
                        handleCreate(newProveedorName, newProveedorCuit);
                        handleClose2();
                      }}
                      type="submit"
                      fullWidth
            ...
            {Proveedor.filter((Proveedor) =>
              ...
              .map((Proveedor) => (
                ...
                >
                  ...
                  >
                    {Proveedor.cuit} {/* Mostrar el CUIT */}
                  </TableCell>
                  <TableCell
                    sx={{
                      background: " #424242",
                  ...
                  >
                    {formatFecha(Proveedor.modifiedDate)}
                  </TableCell>
                  <TableCell align="center">
                    <Box>
                      ...
                      <Button
                        style={{ color: "#fff" }}
                        onClick={() => {
                          setSelectedItemId(Proveedor.id);
                          handleOpen(Proveedor.name, Proveedor.cuit);
                        }}
                      >
                        <EditIcon />
                      </Button>

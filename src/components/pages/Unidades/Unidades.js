...
export default function Unidades() {
  ...
  const [newUnidadesName, setNewUnidadesName] = useState("");
  ...
  const handleDelete = (id) => {
    ...
    fetch(`https://localhost:7244/api/Presupuesto/DeleteUnidades?id=${id}`, {
      method: "DELETE",
    })
      .then((response) => {
        if (response.ok) {
  ...
  const handleEdit = (id, newName) => {
    fetch(
      `https://localhost:7244/api/Presupuesto/UnidadesUpDate?UnidadesId=${id}`,
      {
        method: "PUT",
        headers: {
  ...
  const handleCreate = (newName) => {
    fetch(`https://localhost:7244/api/Presupuesto/Unidades`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      ...
      >
        ...
        >
            ...
            <TableRow sx={{ background: " #424242", color: "#fff" }}>
              ...
              >
                ...
                >
                  <Box sx={style}>
                    <TextField
                      value={newUnidadesName}
                      onChange={(e) => setNewUnidadesName(e.target.value)}
                      margin="normal"
                      fullWidth
                      variant="filled"
                      className="newUnidadesName"
                      name="newUnidadesName"
                      type="newUnidadesName"
                      id="newUnidadesName"
          ...
          <TableBody sx={{ background: " #424242", color: "#fff" }}>
            {Unidades.filter((Unidades) =>
              Unidades.name.toLowerCase().includes(searchTerm.toLowerCase())
            )
              ...
              .map((Unidades) => (
                <TableRow hover role="checkbox" tabIndex={-1} key={Unidades.id}>
                  ...
                  >
                    {formatFecha(Unidades.modifiedDate)}
                  </TableCell>
                  <TableCell align="center">
                    <Box>
                      <Button
                        style={{ color: "#fff" }}
                        onClick={() => handleDelete(Unidades.id)}
                      >
                        <DeleteIcon />
                      </Button>
                      <Button
                        style={{ color: "#fff" }}
                        onClick={() => {
                          setSelectedItemId(Unidades.id);
                          handleOpen(Unidades.name);
                        }}
                      >
                        <EditIcon />
                      </Button>

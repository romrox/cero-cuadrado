...
const columns = [
  ...
  { id: "cuit", label: "CUIT:", minWidth: 100, align: "center" },
...
export default function Empresas() {
  ...
  const [newEmpresaCuit, setNewEmpresaCuit] = useState("");
  ...
  /* editar*/
  const handleOpen = (name, cuit) => {
    setNewEmpresasName(name);
    setNewEmpresaCuit(cuit);
    setOpen(true);
  ...
  const [newEmpresasName, setNewEmpresasName] = useState("");
  ...
  useEffect(() => {
    fetch("https://localhost:7244/api/Presupuesto/GetAllEmpresas")
  ...
  const handleDelete = (id) => {
    ...
    fetch(`https://localhost:7244/api/Presupuesto/DeleteEmpresas?id=${id}`, {
      method: "DELETE",
    })
      .then((response) => {
        if (response.ok) {
          ...
          throw new Error("Error al eliminar la Empresas");
        }
      })
      .catch((error) => console.error("Error al eliminar la Empresas:", error));
  };

  const handleEdit = (id, newName, newCuit) => {
    fetch(
      `https://localhost:7244/api/Presupuesto/EmpresaUpDate?EmpresasId=${id}`,
      {
        method: "PUT",
        headers: {
      ...
      .then((response) => {
        if (response.ok) {
          ...
          setEmpresas(
            Empresas.map((item) =>
              item.id === id ? { ...item, name: newName, cuit: newCuit } : item
            )
          );
  ...
  const handleCreate = (newName, newCuit) => {
    fetch(`https://localhost:7244/api/Presupuesto/Empresas`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      ...
      >
        ...
        >
            ...
            <TableRow sx={{ background: " #424242", color: "#fff" }}>
              ...
              >
                ...
                <Modal open={open} onClose={handleClose}>
                  <Box sx={style}>
                    <TextField
                      value={newEmpresasName}
                      onChange={(e) => setNewEmpresasName(e.target.value)}
                      margin="normal"
                      fullWidth
                      variant="filled"
                      className="newEmpresasName"
                      name="setNewEmpresassName"
                      type="newEmpresasName"
                      id="newEmpresasName"
                    ...
                    />
                    <TextField
                      value={newEmpresaCuit}
                      onChange={(e) => setNewEmpresaCuit(e.target.value)}
                      margin="normal"
                      fullWidth
                      variant="filled"
                      name="newEmpresaCuit"
                ...
                >
                  <Box sx={style}>
                    ...
                    <Button
                      onClick={() => {
                        handleCreate(newEmpresasName, newEmpresaCuit);
                        handleClose2();
                      }}
                      type="submit"
                      fullWidth
          ...
          <TableBody sx={{ background: " #424242", color: "#fff" }}>
            {Empresas.filter((Empresas) =>
              Empresas.name.toLowerCase().includes(searchTerm.toLowerCase())
            )
              .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
              .map((Empresas) => (
                <TableRow hover role="checkbox" tabIndex={-1} key={Empresas.id}>
                  <TableCell
                    sx={{
                      background: " #424242",
                      color: "#fff",
                  ...
                  >
                    {formatFecha(Empresas.modifiedDate)}
                  </TableCell>
                  <TableCell align="center">
                    <Box>
                      <Button
                        style={{ color: "#fff" }}
                        onClick={() => handleDelete(Empresas.id)}
                      >
                        <DeleteIcon />
                      </Button>
                      <Button
                        style={{ color: "#fff" }}
                        onClick={() => {
                          setSelectedItemId(Empresas.id);
                          handleOpen(Empresas.name, Empresas.cuit);
                        }}
                      >
                        <EditIcon />
                      </Button>
